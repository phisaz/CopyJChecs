...
public final class SANUtilsTest {
	...
	/** Chaine FEN d'une allant vers une promotion. */
	private static final String PROMOTION_FEN = "8/k7/8/8/8/8/1p6/7K b - - 10 10";
	...
	public void testSANValidator() {
		...
		assertTrue(regexp.matcher("h1K+").matches());
		assertTrue(regexp.matcher("h1Q++").matches());
		assertTrue(regexp.matcher("h1Q#").matches());
		assertTrue(regexp.matcher("f8Q(=)").matches());
		// Le "=" pour noter une promotion, fréquent dans PGN, ne fait pas
		// partie de
		// SAN.
		...
		// Colonne de départ manquante (obligatoire d'après SAN pour les prises
		// avec
		// pion).
	...
	public void testToMove() {
		...
		try {
			...
			assertEquals(
					new Move(WHITE_PAWN, Square.valueOf("e2"),
							Square.valueOf("e3")), mvt);
			etat = etat.derive(mvt, true);
			mvt = toMove(etat, "Na6");
			assertEquals(
					new Move(BLACK_KNIGHT, Square.valueOf("b8"),
							Square.valueOf("a6")), mvt);
			etat = etat.derive(mvt, true);
			mvt = toMove(etat, "Bc4");
			assertEquals(
					new Move(WHITE_BISHOP, Square.valueOf("f1"),
							Square.valueOf("c4")), mvt);
			etat = etat.derive(mvt, true);
			mvt = toMove(etat, "c5");
			assertEquals(
					new Move(BLACK_PAWN, Square.valueOf("c7"),
							Square.valueOf("c5")), mvt);
			etat = etat.derive(mvt, true);
			mvt = toMove(etat, "Qf3");
			assertEquals(
					new Move(WHITE_QUEEN, Square.valueOf("d1"),
							Square.valueOf("f3")), mvt);
			etat = etat.derive(mvt, true);
			mvt = toMove(etat, "b6");
			assertEquals(
					new Move(BLACK_PAWN, Square.valueOf("b7"),
							Square.valueOf("b6")), mvt);
			etat = etat.derive(mvt, true);
			mvt = toMove(etat, "Qxf7++");
			assertEquals(
					new Move(WHITE_QUEEN, Square.valueOf("f3"),
							Square.valueOf("f7"), BLACK_PAWN), mvt);
			etat = etat.derive(mvt, true);
		...
		try {
			etat = etat.derive(toBoard(PROMOTION_FEN));
			Move mvt = toMove(etat, "b1N");
			assertEquals(
					new Move(BLACK_PAWN, Square.valueOf("b2"),
							Square.valueOf("b1"), null, BLACK_KNIGHT), mvt);

			etat = etat.derive(toBoard(AMBIGUOUS_PAWN_FEN));
			mvt = toMove(etat, "cxb4");
			assertEquals(
					new Move(WHITE_PAWN, Square.valueOf("c3"),
							Square.valueOf("b4"), BLACK_KNIGHT), mvt);

			etat = etat.derive(toBoard(AMBIGUOUS_KNIGHT_FEN));
			mvt = toMove(etat, "Nfxe5");
			assertEquals(
					new Move(WHITE_KNIGHT, Square.valueOf("f3"),
							Square.valueOf("e5"), BLACK_PAWN), mvt);

			etat = etat.derive(toBoard(AMBIGUOUS_QUEEN_FEN));
			mvt = toMove(etat, "Qf5xd5(=)");
			assertEquals(
					new Move(WHITE_QUEEN, Square.valueOf("f5"),
							Square.valueOf("d5"), BLACK_PAWN), mvt);
		} catch (final FENException | SANException e) {
			fail(e.toString());
		}
	}
	...
	public void testToSAN() {
		MoveGenerator etat = BoardFactory.valueOf(ARRAY, STARTING);
		Move mvt = new Move(WHITE_PAWN, Square.valueOf("e2"),
				Square.valueOf("e3"));
		...
		etat = etat.derive(mvt, true);
		mvt = new Move(WHITE_QUEEN, Square.valueOf("f3"), Square.valueOf("f7"),
				BLACK_PAWN);
		assertEquals("Qxf7++", toSAN(etat, mvt));

		try {
			etat = etat.derive(toBoard(PROMOTION_FEN));
			mvt = new Move(BLACK_PAWN, Square.valueOf("b2"),
					Square.valueOf("b1"), null, BLACK_KNIGHT);
			assertEquals("b1N", toSAN(etat, mvt));
			etat = etat.derive(toBoard(AMBIGUOUS_KNIGHT_FEN));
			mvt = new Move(WHITE_KNIGHT, Square.valueOf("f3"),
					Square.valueOf("e5"), BLACK_PAWN);
			assertEquals("Nfxe5", toSAN(etat, mvt));
			etat = etat.derive(toBoard(AMBIGUOUS_PAWN_FEN));
			mvt = new Move(WHITE_PAWN, Square.valueOf("c3"),
					Square.valueOf("b4"), BLACK_KNIGHT);
			assertEquals("cxb4", toSAN(etat, mvt));
			etat = etat.derive(toBoard(AMBIGUOUS_QUEEN_FEN));
			mvt = new Move(WHITE_QUEEN, Square.valueOf("f5"),
					Square.valueOf("d5"), BLACK_PAWN);
