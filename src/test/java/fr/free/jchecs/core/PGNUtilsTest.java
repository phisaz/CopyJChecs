...
public final class PGNUtilsTest {
	...
	public void testToGame() {
		...
		try {
			final Game partie = toGame(new BufferedReader(
					new StringReader(pgn1)));
			Player joueur = partie.getPlayer(true);
			assertEquals("AlphaBeta", joueur.getName());
			assertEquals(EngineFactory.newInstance("jChecs.AlphaBeta")
					.getClass(), joueur.getEngine().getClass());
			...
			assertEquals(
					"rnbqkbnr/ppppppp1/7p/8/P7/2N5/1PPPPPPP/R1BQKBNR b KQkq - 1 2",
					toFEN(partie.getBoard()));
		...
		try {
			...
			assertTrue(partie.getState() == Game.State.IN_PROGRESS);
			assertEquals(
					"rnbqkb1r/pp3ppp/4pn2/2pp4/3P4/2PBP3/PP3PPP/RNBQK1NR w KQkq c6 0 5",
					partie.getStartingPosition());
			assertEquals(
					"rnbqkb1r/pp3ppp/4pn2/2pp4/3PP3/2PB4/PP3PPP/RNBQK1NR b KQkq - 0 5",
					toFEN(partie.getBoard()));
		} catch (final PGNException e) {
			fail(e.toString());
		}
	...
	public void testToPGN() {
		...
		partie1.moveFromCurrent(new Move(WHITE_PAWN, Square.valueOf("a2"),
				Square.valueOf("a4")));
		partie1.moveFromCurrent(new Move(BLACK_PAWN, Square.valueOf("g7"),
				Square.valueOf("g6")));
		partie1.moveFromCurrent(new Move(WHITE_KNIGHT, Square.valueOf("b1"),
				Square.valueOf("c3")));
		...
		final StringBuilder attendu1 = new StringBuilder("[Event \"jChecs v");
		attendu1.append(APPLICATION_VERSION).append(" chess game\"]\n");
		attendu1.append("[Site \"").append(_site).append("\"]\n");
		attendu1.append("[Date \"")
		...
		partie2.moveFromCurrent(new Move(WHITE_PAWN, Square.valueOf("e3"),
				Square.valueOf("e4")));

		final StringBuilder attendu2 = new StringBuilder("[Event \"jChecs v");
		attendu2.append(APPLICATION_VERSION).append(" chess game\"]\n");
		attendu2.append("[Site \"").append(_site).append("\"]\n");
		attendu2.append("[Date \"")
				.append(new SimpleDateFormat("yyyy.MM.dd").format(new Date()))
				.append("\"]\n");
