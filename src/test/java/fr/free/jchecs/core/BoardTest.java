...
public final class BoardTest {
    ...
    public BoardTest() {
        try {
            ...
            LOG.error("Illegal FEN string.", e);
        }
    }
    ...
    public void testGetPieceAt() {
        Board etatPrec = null;
        for (final BoardFactory.Type t : BoardFactory.Type.values()) {
            final Board etat = BoardFactory.valueOf(t,
                    BoardFactory.State.STARTING);
            if (etatPrec != null) {
                final String nomClasse = etat.getClass().getSimpleName();
                for (final Square s : Square.values()) {
                    assertSame(nomClasse, etat.getPieceAt(s),
                            etat.getPieceAt(s.getFile(), s.getRank()));
                    assertSame(nomClasse, etat.getPieceAt(s),
                            etatPrec.getPieceAt(s));
                    assertSame(nomClasse, etat.getPieceAt(s),
                            etatPrec.getPieceAt(s.getFile(), s.getRank()));
                }
            }
            etatPrec = etat;
        }
            ...
            for (final BoardFactory.Type t : BoardFactory.Type.values()) {
                ...
                if (etatPrec != null) {
                    ...
                    assertTrue(nomClasse,
                            etatPrec.hashCode() == etat.hashCode());
                }
                etatPrec = etat;
            }
        }
    ...
    public void testStandard() {
        for (final BoardFactory.Type t : BoardFactory.Type.values()) {
            validateStarting(BoardFactory.valueOf(t,
                    BoardFactory.State.STARTING));
        }
    }
    ...
    public void testToString() {
        final String attendu = "rnbqkbnr\npppppppp\n++++++++\n++++++++\n++++++++\n++++++++\nPPPPPPPP\nRNBQKBNR\n";
        for (final BoardFactory.Type t : BoardFactory.Type.values()) {
            assertEquals(BoardFactory.valueOf(t, BoardFactory.State.STARTING)
                    .toString(), attendu);
        }
    }
    ...
    private static void validateEmpty(final Board pEtat) {
        ...
        assertTrue(nomClasse, pEtat.getPiecesCount() == 0);
    }
    ...
    /**
     * Vérifie que l'état reçu corresponde bien à un état "STARTING".
     *
     * @param pEtat Etat de jeu.
     */
    private static void validateStarting(final Board pEtat) {
        ...
        for (int x = 0; x < FILE_COUNT; x++) {
            ...
            assertSame(nomClasse, pEtat.getPieceAt(x, RANK_COUNT - 2),
                    BLACK_PAWN);
        }
        ...
        assertSame(nomClasse, pEtat.getPieceAt(FILE_COUNT - 1, RANK_COUNT - 1),
                BLACK_ROOK);
        ...
        assertSame(nomClasse, pEtat.getPieceAt(FILE_COUNT - 2, RANK_COUNT - 1),
                BLACK_KNIGHT);
        ...
        assertSame(nomClasse, pEtat.getPieceAt(FILE_COUNT - 3, RANK_COUNT - 1),
                BLACK_BISHOP);
        ...
        assertSame(nomClasse, pEtat.getPieceAt(FILE_COUNT - 4, RANK_COUNT - 1),
                BLACK_KING);

        validateInitialFlags(pEtat);

        assertEquals(nomClasse, Integer.valueOf(32),
                Integer.valueOf(pEtat.getPiecesCount()));
    }
}
