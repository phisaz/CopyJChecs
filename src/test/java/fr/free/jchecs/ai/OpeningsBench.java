...
/**
 * Classe utilitaire permettant de tester l'efficacité de la gestion des
 * ouvertures.
 *
 * @author David Cotton
 */
final class OpeningsBench {
    /**
    ...
    public static void main(final String[] pArgs) {
        System.out.println("Parties croisées (en " + GAMES_COUNT
                + " manches de " + MOVES_COUNT + " coups maximum).");
        ...
        for (int i = 0; i < GAMES_COUNT; i++) {
            MoveGenerator etat = BoardFactory.valueOf(
                    BoardFactory.Type.FASTEST, BoardFactory.State.STARTING);
            while (true) {
                System.out.print('.');
        ...
        System.out.println(" => " + (avecOuvertures / 2.0F)
                + " (Avec ouvertures) / " + (sansOuvertures / 2.0F)
                + " (Sans ouvertures)");
    }
}
